    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                <thead class="table-light"><tr><th>No</th><th>Kode</th><th>Nama Training</th><th>Trainer</th><th>Tanggal Mulai</th><th>Tanggal Selesai</th><th>Lokasi</th><th class="text-center">Kuota</th><th class="text-center">Status</th><th class="text-center">Aksi</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $t->lokasi }}</td>
                        <td class="text-center"><span class="badge bg-primary">{{ $t->peserta }}/{{ $t->kuota }}</span></td>
                        <td class="text-center">@if($t->status=='aktif')<span class="badge bg-success">Aktif</span>@elseif($t->status=='penuh')<span class="badge bg-warning">Penuh</span>@else<span class="badge bg-secondary">Draft</span>@endif</td>
                    ...
                    <div class="modal fade" id="editModal{{ $t->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('training.index.update', $t->id) }}" method="POST">@csrf @method('PUT')
                            <div class="modal-header bg-info text-white"><h5 class="modal-title"><i class="fas fa-edit me-2"></i>Edit Jadwal Training</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                            <div class="modal-body">
                                <div class="mb-3"><label class="form-label">Kode</label><input type="text" class="form-control" name="kode" value="{{ $t->kode }}"></div>
                                <div class="mb-3"><label class="form-label">Nama Training</label><input type="text" class="form-control" name="nama" value="{{ $t->nama }}"></div>
                                ...
                                <div class="row mb-3"><div class="col-md-6"><label class="form-label">Trainer</label><input type="text" class="form-control" name="trainer" value="{{ $t->trainer }}"></div><div class="col-md-6"><label class="form-label">Lokasi</label><input type="text" class="form-control" name="lokasi" value="{{ $t->lokasi }}"></div></div>
                                <div class="row mb-3"><div class="col-md-6"><label class="form-label">Tanggal Mulai</label><input type="date" class="form-control" name="tanggal_mulai" value="{{ $t->tanggal_mulai }}"></div><div class="col-md-6"><label class="form-label">Tanggal Selesai</label><input type="date" class="form-control" name="tanggal_selesai" value="{{ $t->tanggal_selesai }}"></div></div>
                                <div class="row mb-3"><div class="col-md-6"><label class="form-label">Kuota</label><input type="number" class="form-control" name="kuota" value="{{ $t->kuota }}"></div><div class="col-md-6"><label class="form-label">Status</label><select class="form-select" name="status"><option value="aktif" @if($t->status=='aktif')selected@endif>Aktif</option><option value="penuh" @if($t->status=='penuh')selected@endif>Penuh</option><option value="draft" @if($t->status=='draft')selected@endif>Draft</option></select></div></div>
                            </div>
                    ...
                    <div class="modal fade" id="deleteModal{{ $t->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('training.index.destroy', $t->id) }}" method="POST">@csrf @method('DELETE')
                    ...
                    @empty<tr><td colspan="10" class="text-center py-5 text-muted"><i class="fas fa-graduation-cap fa-3x mb-3 d-block"></i><p>Belum ada jadwal training</p></td></tr>@endforelse
                </tbody>
            </table>
        </div>
    </div>
...
<div class="modal fade" id="addModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form action="{{ route('training.index.store') }}" method="POST">@csrf
        <div class="modal-header bg-primary text-white"><h5 class="modal-title"><i class="fas fa-plus me-2"></i>Tambah Jadwal Training</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            ...
            <div class="mb-3"><label class="form-label">Nama Training <span class="text-danger">*</span></label><input type="text" class="form-control" name="nama" required></div>
            <div class="mb-3"><label class="form-label">Deskripsi</label><textarea class="form-control" name="deskripsi" rows="2"></textarea></div>
            <div class="row mb-3"><div class="col-md-6"><label class="form-label">Trainer</label><input type="text" class="form-control" name="trainer"></div><div class="col-md-6"><label class="form-label">Lokasi</label><input type="text" class="form-control" name="lokasi"></div></div>
            ...
            <div class="mb-3"><label class="form-label">Kuota</label><input type="number" class="form-control" name="kuota"></div>
        </div>
