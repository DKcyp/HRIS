        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <div class="modal fade" id="editModal{{ $a->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('asset.index.update', $a->id) }}" method="POST">@csrf @method('PUT')
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row mb-3"><div class="col-md-6"><label class="form-label">Kategori</label><select class="form-select" name="kategori"><option value="Elektonik" {{ $a->kategori=='Elektonik' ? 'selected' : '' }}>Elektonik</option><option value="Furniture" {{ $a->kategori=='Furniture' ? 'selected' : '' }}>Furniture</option><option value="Kendaraan" {{ $a->kategori=='Kendaraan' ? 'selected' : '' }}>Kendaraan</option></select></div><div class="col-md-6"><label class="form-label">Merek</label><input type="text" class="form-control" name="merek" value="{{ $a->merek }}"></div></div>
                                ...
                                <div class="row mb-3"><div class="col-md-4"><label class="form-label">Kondisi</label><select class="form-select" name="kondisi"><option value="baik" {{ $a->kondisi=='baik' ? 'selected' : '' }}>Baik</option><option value="rusak_ringan" {{ $a->kondisi=='rusak_ringan' ? 'selected' : '' }}>Rusak Ringan</option><option value="rusak_berat" {{ $a->kondisi=='rusak_berat' ? 'selected' : '' }}>Rusak Berat</option></select></div><div class="col-md-4"><label class="form-label">Lokasi</label><input type="text" class="form-control" name="lokasi" value="{{ $a->lokasi }}"></div><div class="col-md-4"><label class="form-label">Status</label><select class="form-select" name="status"><option value="tersedia" {{ $a->status=='tersedia' ? 'selected' : '' }}>Tersedia</option><option value="dipinjam" {{ $a->status=='dipinjam' ? 'selected' : '' }}>Dipinjam</option></select></div></div>
                            </div>
                    ...
                    @empty
                    <tr><td colspan="11" class="text-center py-5 text-muted"><i class="fas fa-laptop fa-3x mb-3 d-block"></i><p>Belum ada data asset</p></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
