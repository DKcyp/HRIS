@extends('layouts.app')
@section('title', 'Birthday - HRIS V2')
@section('page-title', 'Data Ulang Tahun Karyawan')
...
<div class="card">
    <div class="card-header bg-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class="fas fa-birthday-cake me-2"></i>Data Ulang Tahun Karyawan</h5>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                <thead class="table-light"><tr><th>No</th><th>NIK</th><th>Nama</th><th>Tanggal Lahir</th><th>Usia</th><th>Divisi</th><th>Jabatan</th><th class="text-center">Aksi</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge bg-primary">{{ $b->usia }} Tahun</span></td>
                        <td>{{ $b->divisi }}</td>
                        <td>{{ $b->jabatan }}</td>
                    ...
                    <div class="modal fade" id="editModal{{ $b->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('pengumuman.birthday.update', $b->id) }}" method="POST">@csrf @method('PUT')
                            <div class="modal-header bg-info text-white"><h5 class="modal-title"><i class="fas fa-edit me-2"></i>Edit Birthday</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                            <div class="modal-body">
                                ...
                                <div class="mb-3"><label class="form-label">Tanggal Lahir</label><input type="date" class="form-control" name="tanggal_lahir" value="{{ $b->tanggal_lahir }}"></div>
                                <div class="row mb-3"><div class="col-md-6"><label class="form-label">Divisi</label><input type="text" class="form-control" name="divisi" value="{{ $b->divisi }}"></div><div class="col-md-6"><label class="form-label">Jabatan</label><input type="text" class="form-control" name="jabatan" value="{{ $b->jabatan }}"></div></div>
                            </div>
                    ...
                    <div class="modal fade" id="deleteModal{{ $b->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('pengumuman.birthday.destroy', $b->id) }}" method="POST">@csrf @method('DELETE')
...
<div class="modal fade" id="addModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form action="{{ route('pengumuman.birthday.store') }}" method="POST">@csrf
        <div class="modal-header bg-primary text-white"><h5 class="modal-title"><i class="fas fa-plus me-2"></i>Tambah Birthday</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
