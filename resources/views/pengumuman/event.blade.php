        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <div class="modal fade" id="editModal{{ $e->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('pengumuman.event.update', $e->id) }}" method="POST">@csrf @method('PUT')
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3"><label class="form-label">Status</label><select class="form-select" name="status"><option value="aktif" @if($e->status=='aktif')selected@endif>Aktif</option><option value="selesai" @if($e->status=='selesai')selected@endif>Selesai</option><option value="draft" @if($e->status=='draft')selected@endif>Draft</option></select></div>
                            </div>
                    ...
                    @empty<tr><td colspan="10" class="text-center py-5 text-muted"><i class="fas fa-calendar-day fa-3x mb-3 d-block"></i><p>Belum ada data event</p></td></tr>@endforelse
                </tbody>
            </table>
        </div>
    </div>
