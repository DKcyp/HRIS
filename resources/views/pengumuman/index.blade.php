        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <div class="modal fade" id="editModal{{ $n->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('pengumuman.index.update', $n->id) }}" method="POST">@csrf @method('PUT')
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row mb-3"><div class="col-md-6"><label class="form-label">Kategori</label><select class="form-select" name="kategori"><option value="Penting" {{ $n->kategori=='Penting' ? 'selected' : '' }}>Penting</option><option value="Pengumuman" {{ $n->kategori=='Pengumuman' ? 'selected' : '' }}>Pengumuman</option><option value="Training" {{ $n->kategori=='Training' ? 'selected' : '' }}>Training</option><option value="Teknis" {{ $n->kategori=='Teknis' ? 'selected' : '' }}>Teknis</option></select></div><div class="col-md-6"><label class="form-label">Status</label><select class="form-select" name="status"><option value="aktif" {{ $n->status=='aktif' ? 'selected' : '' }}>Aktif</option><option value="draft" {{ $n->status=='draft' ? 'selected' : '' }}>Draft</option></select></div></div>
                            </div>
                    ...
                    @empty
                    <tr><td colspan="9" class="text-center py-5 text-muted"><i class="fas fa-bullhorn fa-3x mb-3 d-block"></i><p>Belum ada data pengumuman</p></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
