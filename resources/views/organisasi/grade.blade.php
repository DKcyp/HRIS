        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button class="btn btn-sm btn-outline-info me-1" title="Edit"
                                data-bs-toggle="modal" data-bs-target="#editModal{{ $g->id }}">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger" title="Hapus"
                                data-bs-toggle="modal" data-bs-target="#deleteModal{{ $g->id }}">
                    ...
                    <div class="modal fade" id="editModal{{ $g->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('organisasi.grade.update', $g->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" name="kode" value="{{ $g->kode }}" required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Nama Grade <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control" name="nama" value="{{ $g->nama }}" required>
                                        </div>
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                <label class="form-label">Gaji Minimum</label>
                                                <input type="number" class="form-control" name="min_gaji" value="{{ $g->min_gaji }}">
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Gaji Maksimum</label>
                                                <input type="number" class="form-control" name="max_gaji" value="{{ $g->max_gaji }}">
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" name="status">
                                                ...
                                                <option value="non-aktif" {{ $g->status == 'non-aktif' ? 'selected' : '' }}>Non-aktif</option>
                                            </select>
                                        </div>
                                    </div>
                    ...
                    <!-- Delete Modal -->
                    <div class="modal fade" id="deleteModal{{ $g->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('organisasi.grade.destroy', $g->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <div class="modal-header bg-danger text-white">
                                        <h5 class="modal-title"><i class="fas fa-trash me-2"></i>Hapus Grade</h5>
                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body text-center py-4">
                                        <i class="fas fa-exclamation-triangle fa-4x text-warning mb-3"></i>
                                        <p class="fs-5">Yakin ingin menghapus grade ini?</p>
                                        <p class="text-muted"><strong>{{ $g->nama }}</strong></p>
                                    </div>
...
<div class="modal fade" id="addModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('organisasi.grade.store') }}" method="POST">
                @csrf
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title"><i class="fas fa-plus me-2"></i>Tambah Grade</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label class="form-label">Gaji Minimum</label>
                            <input type="number" class="form-control" name="min_gaji">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Gaji Maksimum</label>
                            <input type="number" class="form-control" name="max_gaji">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Deskripsi</label>
