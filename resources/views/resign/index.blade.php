        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <div class="modal fade" id="editModal{{ $r->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('resign.index.update', $r->id) }}" method="POST">@csrf @method('PUT')
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3"><label class="form-label">Status</label><select class="form-select" name="status"><option value="disetujui" @if($r->status=='disetujui')selected@endif>Disetujui</option><option value="pending" @if($r->status=='pending')selected@endif>Pending</option><option value="ditolak" @if($r->status=='ditolak')selected@endif>Ditolak</option></select></div>
                            </div>
                    ...
                    @empty<tr><td colspan="10" class="text-center py-5 text-muted"><i class="fas fa-door-open fa-3x mb-3 d-block"></i><p>Belum ada data pengajuan resign</p></td></tr>@endforelse
                </tbody>
            </table>
        </div>
    </div>
