        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <div class="modal fade" id="editModal{{ $c->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        <form action="{{ route('resign.clearance.update', $c->id) }}" method="POST">@csrf @method('PUT')
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row mb-3"><div class="col-md-3"><label class="form-label">IT</label><select class="form-select" name="it_clean"><option value="Ya" @if($c->it_clean=='Ya')selected@endif>Ya</option><option value="Belum" @if($c->it_clean=='Belum')selected@endif>Belum</option></select></div><div class="col-md-3"><label class="form-label">Finance</label><select class="form-select" name="finance_clean"><option value="Ya" @if($c->finance_clean=='Ya')selected@endif>Ya</option><option value="Belum" @if($c->finance_clean=='Belum')selected@endif>Belum</option></select></div><div class="col-md-3"><label class="form-label">HRD</label><select class="form-select" name="hrd_clean"><option value="Ya" @if($c->hrd_clean=='Ya')selected@endif>Ya</option><option value="Belum" @if($c->hrd_clean=='Belum')selected@endif>Belum</option></select></div><div class="col-md-3"><label class="form-label">Office</label><select class="form-select" name="office_clean"><option value="Ya" @if($c->office_clean=='Ya')selected@endif>Ya</option><option value="Belum" @if($c->office_clean=='Belum')selected@endif>Belum</option></select></div></div>
                            </div>
                    ...
                    @empty<tr><td colspan="12" class="text-center py-5 text-muted"><i class="fas fa-clipboard-check fa-3x mb-3 d-block"></i><p>Belum ada data clearance</p></td></tr>@endforelse
                </tbody>
            </table>
        </div>
    </div>
